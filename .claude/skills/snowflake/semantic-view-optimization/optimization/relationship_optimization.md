...
parent_skill: semantic-view-optimization-patterns
priority: 3
---
...
```bash
uv run python ../scripts/semantic_view_get.py --file model.yaml --table TABLE_NAME --component primary_key
```
...
```bash
SNOWFLAKE_CONNECTION_NAME=<conn> uv run python ../scripts/infer_primary_keys.py \
...
```bash
uv run python ../scripts/semantic_view_set.py \
  --input-file model.yaml --output-file model.yaml \
